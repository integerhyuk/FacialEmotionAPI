...
async def process_video(file: UploadFile, sampling_rate: int):
    ...
    return {file.filename: grouped_averages_with_time}


@app.post("/recognize/")
async def recognize_videos(files: List[UploadFile] = File(...), sampling_rate: int = 1):
    if not all(file.content_type.startswith("video/") for file in files):
        raise HTTPException(status_code=400, detail="This API supports only video files.")

    results = await asyncio.gather(*[process_video(file, sampling_rate) for file in files])
    return {"videos_analysis": results}
